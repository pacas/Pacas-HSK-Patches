...
namespace VoidEvents
{
	public class SymbolResolver_EdgeDefense : SymbolResolver
	{
		...
		public override void Resolve(ResolveParams rp)
		{
			...
			{
				width = rp.edgeDefenseMortarsCount.HasValue && rp.edgeDefenseMortarsCount.Value > 0 ? 4 : Rand.Bool ? 2 : 4;
			}
			...
			if (num > 0)
			{
				...
				for (int i = 0; i < num; i++)
				{
					PawnGenerationRequest value = new PawnGenerationRequest(faction.RandomPawnKind(), faction, PawnGenerationContext.NonPlayer, -1,
						forceGenerateNewPawn: false, allowDead: false, allowDowned: false, canGeneratePawnRelations: true, mustBeCapableOfViolence: true);
					ResolveParams resolveParams = rp;
					resolveParams.faction = faction;
					resolveParams.singlePawnLord = singlePawnLord;
					resolveParams.singlePawnGenerationRequest = value;
					resolveParams.singlePawnSpawnCellExtraPredicate = resolveParams.singlePawnSpawnCellExtraPredicate ?? delegate (IntVec3 x)
					{
