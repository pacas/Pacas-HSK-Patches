	...
	public class GenStep_Power : GenStep
	{
		...
		private void EnsureBatteriesConnectedAndMakeSense(Map map)
		{
			...
			for (int i = 0; i < tmpThings.Count; i++)
			{
				...
				else if (canSpawnPowerGenerators && TrySpawnPowerGeneratorNear(compPowerBattery.parent.Position, map, compPowerBattery.parent.Faction, out Building newPowerGenerator2))
				{
		...
		private void EnsurePowerUsersConnected(Map map)
		{
			...
			for (int i = 0; i < tmpThings.Count; i++)
			{
				...
				map.powerNetManager.UpdatePowerNetsAndConnections_First();
				if (TryFindClosestReachableNet(powerComp.parent.Position, (PowerNet x) => x.CurrentEnergyGainRate() -
				(powerComp.Props.PowerConsumption * CompPower.WattsToWattDaysPerTick) > 1E-07f, map, out PowerNet foundNet, out IntVec3 closestTransmitter))
				{
					map.floodFiller.ReconstructLastFloodFillPath(closestTransmitter, tmpCells);
					bool flag = false;
				...
				else if (canSpawnBatteries && TrySpawnBatteryNear(tmpThings[i].Position, map, tmpThings[i].Faction, out Building newBattery))
				{
		...
		private bool IsPowerUser(Thing thing)
		{
			...
			if (compPowerTrader != null)
			{
				...
				{
					return !compPowerTrader.PowerOn && compPowerTrader.Props.PowerConsumption > 0f;
				}
				return true;
			}
			return false;
